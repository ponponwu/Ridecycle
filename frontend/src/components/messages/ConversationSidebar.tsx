...
interface ConversationSidebarProps {
    ...
    currentConversationId: string | null // Changed from currentBicycleId and allow null
    onSelectConversation: (id: string) => void
    className?: string // Added className to allow dynamic styling from parent
}

const ConversationSidebar = ({
    conversations,
    currentConversationId, // Changed from currentBicycleId
    ...
    return (
        <div className={`bg-white border-r w-full md:w-1/3 lg:w-1/4 flex-shrink-0 ${className}`}>
            {' '}
            {/* Applied className */}
            ...
            <div className="overflow-y-auto h-[calc(100%-60px)]">
                {conversations.length === 0 && (
                    <div className="p-4 text-center text-gray-500">{t('youHaveNoConversations')}</div>
                )}
                {conversations.map((conversation) => (
                    <div
                        ...
                        className={`p-4 border-b flex cursor-pointer hover:bg-gray-50 transition-colors ${
                            currentConversationId === conversation.id ? 'bg-gray-100' : '' // Changed from currentBicycleId
                        }`}
                    >
